...
10   ifield=ifield+1
  ...
  if(ifield.eq.1) then
    ...
    if((xaux1.eq.0.).and.(xaux2.eq.-1.0)) then ! NCEP DATA FROM 0 TO
      xaux1=-179.0                             ! 359 DEG EAST ->
      xaux2=180.0                              ! TRANSFORMED TO -179
  ...
  if((isec1(6).eq.007).and.(isec1(7).eq.001)) then
    do jy=0,ny-1
      do ix=0,nxfield-1
        ...
        if(ix.le.180) then
          oro(179+ix,jy)=help
          excessoro(179+ix,jy)=0.0 ! ISOBARIC SURFACES: SUBGRID TERRAIN DISREGARDED
        else
          oro(ix-181,jy)=help
          excessoro(ix-181,jy)=0.0 ! ISOBARIC SURFACES: SUBGRID TERRAIN DISREGARDED
        endif
      end do
    end do
  endif
...
30   call pbclose(lunit,ierr)     !! FINISHED READING / CLOSING GRIB FILE
  ...
  write(*,'(a,f10.1,a1,f10.1,a,f10.1)') '  Longitude range: ', &
       xlon0,' to ',xlon0+(nx-1)*dx,'   Grid distance: ',dx
  write(*,'(a,f10.1,a1,f10.1,a,f10.1)') '  Latitude range: ', &
