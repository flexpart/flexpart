...
subroutine partoutput(itime)
  ...
  if (ipout.eq.1) then
      open(unitpartout,file=path(2)(1:length(2))//'partposit_'//adate// &
         atime,form='unformatted') ! for testreasons  modified by mc it should be unformatted 
   ! open(unitpartout,file=path(2)(1:length(2))//'partposit_'//adate// &
   !      atime,form='formatted') ! for test reasons  modified by mc it should be unformatted 
  else
    open(unitpartout,file=path(2)(1:length(2))//'partposit_end', &
         form='unformatted') ! for testreasons  modified by mc it should be unformatted 
   ! open(unitpartout,file=path(2)(1:length(2))//'partposit_end', &
   !      form='formatted')    ! for test reasons  modified by mc it should be unformatted 
  endif

  ! Write current time to file
  !***************************

  !write(unitpartout) itime !commented for testing reason by mc
  do i=1,numpart
...
6     continue
  ...
  !*****************
      ...
      write(unitpartout) npoint(i),xlon,ylat,ztra1(i), & !original writing comemnted out for test reason by mc
           itramem(i),topo,pvi,qvi,rhoi,hmixi,tri,tti, &
           (xmass1(i,j),j=1,nspec)
     !write(unitpartout,*)xlon,ylat,ztra1(i) !modified for testreaosn by mc, to be removed and but as above
     
    endif
  end do
  write(unitpartout) -99999,-9999.9,-9999.9,-9999.9,-99999, &   !comemnted out for test reason by mc|!
