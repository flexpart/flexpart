...
subroutine releaseparticles(itime)
  ...
  !     29 June 2002                                                           *
  !                                                                            *
  !     modfied by M.Cassiani                                                  *
  !     for FLEXPART-NorESM no weekly cycle allowed                             *
  !                                                                            *
  !*****************************************************************************
  !                                                                            *
  ! Variables:                                                                 *
  ...
  integer :: idummy = -7
  integer :: flaginit=0 ! for NORESM added by mc 
  ...
  julmonday=juldate(19000101,0)              ! this is a Monday !not used and commented out here for NORESM, comment by mc
  jul=bdate+real(itime,kind=dp)/86400._dp    ! this is the current day
  if (flaginit.eq.0) then
      !julmonday=jul ! for NORESM comment by mc
      flaginit=1     ! for NORESM added by mc
  end if
  call caldate(jul,jjjjmmdd,ihmmss)
  mm=(jjjjmmdd-10000*(jjjjmmdd/10000))/100
  !if ((mm.ge.4).and.(mm.le.9)) jul=jul+1._dp/24._dp   ! daylight savings time in summer !this is not used in the NORESM version of the code, comment by mc
