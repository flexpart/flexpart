...
subroutine readavailable
  ...
  !     adapted for NorESM output with no-leap calendar                        *
  !                                                                            *
  !*****************************************************************************
  !                                                                            *
  ! Variables:                                                                 *
  ...
  implicit none
  !integer YYYYMMDD,HHMISS !added by mc mc for test reason
...
100   read(unitavailab,'(i8,1x,i6,2(6x,a255))',end=99) &
    ...
    if (ldat.lt.yearorigin)    then                                        !modified by mc
    write(*,*) ' #### FLEXPART MODEL ERROR! date                  ####'    !modified by mc
    write(*,*) ' #### must be gt yearorign: Difference in time    ####'    !modified by mc
    write(*,*) ' #### computed using no-leap calendar from an     ####'    !modified by mc
    write(*,*) ' #### arbitrary year zero                         #### '   !modified by mc
    write(*,*) ' #### "AVAILABLE FILE".                           #### '   !modified by mc
    stop
    end if
    jul=juldate(ldat,ltim)
    !call CALDATE(JUL,YYYYMMDD,HHMISS) !added by mc for test reason 
