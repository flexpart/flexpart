...
subroutine readspecies(id_spec,pos_spec)
  ...
  integer :: readerror, puse_NH3_loss, pspec_ass
...
! declare namelist
  namelist /species_params/ &
       ...
       pdsigma, pdryvel, pweightmolar, pohcconst, & 
       pohdconst, pohnconst, pspec_ass, pkao, puse_NH3_loss
...
!       write(*,*) kao(pos_spec)
    ...
    kao(pos_spec)=pkao
    use_NH3_loss(pos_spec)=puse_NH3_loss
...
! Check scavenging parameters given in SPECIES file
  ...
  if (use_NH3_loss(pos_spec).ne.0) then
     if (lroot) &
        & write(*,*) 'NH3 loss rates applied'
     NH3LOSS=.true.
  endif
