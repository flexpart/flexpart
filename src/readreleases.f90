...
subroutine readreleases
  ...
  !     HSO, 12 August 2013: Added optional namelist input                     *
  !     PS, 2/2015: access= -> position= (ported from 9.2 review)              *
  !     PS, 8/2018: check for uninitialised mass (ticket:204),                 *
  !          rename int/real variables, a bit of code layout improvement       *
  !                                                                            *
  !*****************************************************************************
  !                                                                            *
  ! Variables:                                                                 *
  ...
  integer :: numpartmax,i,j,id1,it1,id2,it2,idum,iallocstat,irel,ispc,nsettle
  ...
  integer :: numpoints, nparts, ios
  integer*2 :: ikindz
  integer :: idate1, itime1, idate2, itime2
  real :: rlon1,rlon2,rlat1,rlat2,z1,z2
  character*40 :: comment
  integer,parameter :: unitreleasesout=2
  real,allocatable, dimension (:) :: rmass
  integer,allocatable, dimension (:) :: numspec_rel,numspec_rel2
...
! allocate with maxspec for first input loop
  allocate(rmass(maxspec),stat=iallocstat)
  if (iallocstat.ne.0) write(*,*)'ERROR: could not allocate rmass'
  allocate(numspec_rel(maxspec),stat=iallocstat)
  if (iallocstat.ne.0) write(*,*)'ERROR: could not allocate numspec_rel'
...
! prepare namelist output if requested
  if (nmlout.and.lroot) then
    open(unitreleasesout,file=path(2)(1:length(2))//'RELEASES.namelist', &
      position='append',status='replace',err=1000)
  endif

! check whether namelist input is available
  read(unitreleases,releases_ctrl,iostat=ios)

  if (ios .ne. 0 .or. nspec .lt. 0 ) then

  ! not in namelist format, close file and reopen as simple text file

    close(unitreleases)
    open(unitreleases,file=path(1)(1:length(1))//'RELEASES', status='old', &
      err=999)

    ios=1 ! indicates old format

  ! Check the format of the RELEASES file
  ! (either in free format, or using a formatted mask)
  ! Use of formatted mask is assumed if line 10 contains the word 'DIRECTION'
  !**************************************************************************
! Note PS: something has changed, now Total is checked. Is this compatible??
...
901 format (a)
  ...
  !**********************************
    ...
    do i=1,nspec
      read(unitreleases,*,err=998) numspec_rel(i)
      if (old) call skplin(2,unitreleases)
    end do
...
25  numpoint=numpoint-1
  ...
  else ! read namelist input

    ios=0
    do while (ios.eq.0)  ! loop over release points
      idate1=-1
...
! allocate arrays of matching size for number of species (namelist output)
  deallocate(rmass)
  allocate(rmass(nspec),stat=iallocstat)
  if (iallocstat.ne.0) write(*,*)'ERROR: could not allocate rmass'
  allocate(numspec_rel2(nspec),stat=iallocstat)
  if (iallocstat.ne.0) write(*,*)'ERROR: could not allocate numspec_rel2'

  numspec_rel2 = numspec_rel(1:nspec)

  deallocate(numspec_rel)
  ! eso: BUG, crashes here for nspec=12 and maxspec=6,
  ! TODO: catch error and exit
  allocate(numspec_rel(nspec),stat=iallocstat)
  if (iallocstat.ne.0) write(*,*)'ERROR: could not allocate numspec_rel'

  numspec_rel = numspec_rel2

  deallocate(numspec_rel2)

! allocate memory for numpoint releaspoints
  allocate ( &
    ireleasestart(numpoint), ireleaseend(numpoint),xpoint1(numpoint), &
    xpoint2(numpoint), ypoint1(numpoint), ypoint2(numpoint), &
    zpoint1(numpoint), zpoint2(numpoint), kindz(numpoint), &
    xmass(numpoint,maxspec), rho_rel(numpoint), npart(numpoint), &
    xmasssave(numpoint), &
      stat=iallocstat )
  if (iallocstat.ne.0) write(*,*)'ERROR: could not allocate variables' // &
    ' dimensioned numpoint =', numpoint

  if (lroot) write (*,*) 'Release points : ', numpoint
...
! namelist output
  if (nmlout.and.lroot) write(unitreleasesout,nml=releases_ctrl)
...
! species loop
  do i=1,nspec
    ...
    if (ios.ne.0) then
      read(unitreleases,*,err=998) numspec_rel(i)
      if (old) call skplin(2,unitreleases)
      call readspecies(numspec_rel(i),i)
    else
      call readspecies(numspec_rel(i),i)
    endif
...
! Note PS: this should be replace by = alog(2.)/decay(i)
! keep it for the moment so that results are not affect in test phase of v10
  ...
  !****************************
    ...
    vsetaver(i) = 0.
    cunningham(i) = 0.
    dquer(i) = dquer(i)*1.e6 ! Conversion m to um
  ...
  ! ESO 04.2016 check for below-cloud scavenging (gas or aerosol)
    if ( &
      dquer(i).le.0. .and. (weta_gas(i)  .gt.0. .or. wetb_gas(i)  .gt.0.) .or. &
      dquer(i).gt.0. .and. (crain_aero(i).gt.0. .or. csnow_aero(i).gt.0.)  &
       )  then
      wetdep        = .true.
      wetdepspec(i) = .true.
      if (lroot) then
...
101 numpoint=numpoint+1
  ...
  if (ios.lt.1) then ! reading namelist format

    if (numpoint.gt.numpoints) goto 250
    ikindz = 1
    rmass = -1. ! initialise as negative to catch if not given in input
    nparts = 0
    comment = ' '
    read(unitreleases,release,iostat=ios)
    id1=idate1
    ...
    id2=idate2
    it2=itime2
    xpoint1(numpoint)=rlon1
    xpoint2(numpoint)=rlon2
    ypoint1(numpoint)=rlat1
    ypoint2(numpoint)=rlat2
    zpoint1(numpoint)=z1
    zpoint2(numpoint)=z2
    kindz(numpoint)=ikindz
    do i=1,nspec
      if (rmass(i) .lt. 0.) goto 995 ! write error and stop
      xmass(numpoint,i)=rmass(i)
    end do
    npart(numpoint)=nparts
  ...
  ! namelist output
    if (nmlout.and.lroot) then
      ...
      idate2=id2
      itime2=it2
      rlon1=xpoint1(numpoint)
      rlon2=xpoint2(numpoint)
      rlat1=ypoint1(numpoint)
      rlat2=ypoint2(numpoint)
      z1=zpoint1(numpoint)
      z2=zpoint2(numpoint)
      ikindz=kindz(numpoint)
      nparts=npart(numpoint)
      write(unitreleasesout,nml=release)
    endif
...
250 close(unitreleases)
  ...
  if (mquasilag.ne.0) then
    ...
    do irel=1,numpoint
      nsettle=0
      do ispc=1,nspec
        if (xmass(irel,ispc).gt.tiny(1.)) nsettle=nsettle+1
      end do
  ...
  if (lroot) then
    if (.not.lsettling) &
     write(*,*) 'WARNING: more than 1 species per release point,' &
           // ' settling disabled'
  end if
  ...
  if (releaserate .gt. 0.99*real(maxpart)/real(lage(nageclass))) then
    if (numpartmax.gt.maxpart.and.lroot) then
      ...
      write(*,*) 'Total number of particles released during the simulation is:'&
        ,numpartmax
  ...
  if (lroot) then
    write(*,FMT='(A,ES14.7)') ' Total mass released:', &
      sum(xmass(1:numpoint,1:nspec))
  end if
...
995 write(*,*) '#####################################################'
  write(*,*) '#### FLEXPART MODEL SUBROUTINE READRELEASES:     ####'
  write(*,*) '####                                             ####'
  write(*,*) '#### ERROR - mass for species',i,' not given     ####'
  write(*,*) '#####################################################'
  stop 'readreleases - undefined mass'
